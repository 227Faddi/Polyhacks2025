...
@app.get("/planet_by_name:{name}")
async def planet_name(name):
    request_url = "https://api.le-systeme-solaire.net/rest/bodies" + f"/{name}"

    response = await fetch_data(request_url)

    if response.status == "success":
        return response.data
    elif response.status == "error":
        return "An Request Error Occured" + " : " + response.message
    else:
